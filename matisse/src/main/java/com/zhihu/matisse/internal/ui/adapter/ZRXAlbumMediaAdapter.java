...
import com.zhihu.matisse.internal.ui.widget.ZRXMediaGrid;
import com.zhihu.matisse.internal.utils.PathUtils;

public class ZRXAlbumMediaAdapter extends
        RecyclerViewCursorAdapter<RecyclerView.ViewHolder> implements
        ZRXMediaGrid.OnMediaGridClickListener {
    ...
    public ZRXAlbumMediaAdapter(Context context, SelectedItemCollection selectedCollection, RecyclerView recyclerView) {
        super(null);
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_CAPTURE) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.photo_capture_item_zrx, parent, false);
            ...
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.media_grid_item_zrx, parent, false);
...
//            captureViewHolder.mHint.setCompoundDrawables(drawables[0], drawables[1], drawables[2], drawables[3]);
            ...
            mediaViewHolder.mMediaGrid.preBindMedia(new ZRXMediaGrid.PreBindInfo(
    ...
    private void setCheckStatus(Item item, ZRXMediaGrid mediaGrid) {
    ...
    @Override
    public void onCheckViewClicked(ZRXCheckView checkView, Item item, RecyclerView.ViewHolder holder) {
        if (mSelectionSpec.countable) {
            int checkedNum = mSelectedCollection.checkedNumOf(item);
            if (checkedNum == CheckView.UNCHECKED) {
                if (assertAddSelection(holder.itemView.getContext(), item)) {
                    //
                    String path = PathUtils.getPath(mContext, item.uri);
                    if (!TextUtils.isEmpty(path) && SelectorUtils.isFileSizeOk(path)) {
                        mSelectedCollection.add(item);
                        notifyCheckStateChanged();
                    }else {
                        Toast.makeText(mContext, "视频不能超过50M", Toast.LENGTH_SHORT).show();
                    }
                }
            } else {
    ...
    private static class MediaViewHolder extends RecyclerView.ViewHolder {
        ...
        private ZRXMediaGrid mMediaGrid;

        MediaViewHolder(View itemView) {
            super(itemView);
            mMediaGrid = (ZRXMediaGrid) itemView;
        }
    }
