...
public final class PhotoMetadataUtils {
    ...
    public static Point getBitmapSize(Uri uri, String path, Activity activity) {
        ContentResolver resolver = activity.getContentResolver();
        Point imageSize = getBitmapBound(resolver, uri);
        int w = imageSize.x;
        int h = imageSize.y;
        if (PhotoMetadataUtils.shouldRotate(resolver, path)) {
            w = imageSize.y;
            h = imageSize.x;
        }
    ...
    private static boolean shouldRotate(ContentResolver resolver, String path) {
        ExifInterface exif;
        try {
            exif = ExifInterfaceCompat.newInstance(path);
        } catch (IOException e) {
            Log.e(TAG, "could not read exif info of the image: ");
            return false;
        }
        int orientation = exif.getAttributeInt(ExifInterface.TAG_ORIENTATION, -1);
        return orientation == ExifInterface.ORIENTATION_ROTATE_90
