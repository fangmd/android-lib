...
import com.lhjx.webviewlib.R;
import com.passon.webviewlib.jsbridge.BridgeWebView;
import com.passon.webviewlib.jsbridge.BridgeWebViewClient;

/**
 * Author: Created by fangmingdong on 2018/4/24-上午9:04
 * Description: 集成 JSBridge 的 WebView
 */
public class MyWebViewJsBridge extends BridgeWebView {
    ...
    private void initWebViewSettings() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            setWebContentsDebuggingEnabled(WebViewUtils.sIsDEBUG);
        }
...
//        getSettingsExtension().setPageCacheCapacity(IX5WebSettings.DEFAULT_CACHE_CAPACITY);//extension
        ...
        // 隐藏滚动条, 默认就是隐藏的，如果需要开启将下面设置陈 true
...
//        setScrollBarStyle(View.SCROLLBARS_INSIDE_OVERLAY); //滚动条在WebView外侧显示
    ...
    private void addProgressBar(@DrawableRes int drawable) {
        ...
        mProgressBar.setLayoutParams(new ViewGroup.LayoutParams(LayoutParams.MATCH_PARENT, 5));
